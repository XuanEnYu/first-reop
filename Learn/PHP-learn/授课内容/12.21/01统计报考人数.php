                ...
                <div class="table">
                    <?php
                        ...
                        for($i=0;$i<10;$i++){
                            ...
                            // echo $color;
                            echo '<div>
                                <div class="year" style="background:'.$color.'">'.$arr[$i+1][0].'</div>
                                <div>'.$arr[$i+1][1].'</div>
                                <div class="level">';
                                ball((int)($arr[$i+1][1]/11000),$color);
                            echo '</div>
                            </div>';
                        }
                        // 定义一个生成小球的函数。传两个参数，一个是球的数量，一个是球的颜色
                        function ball($num,$color){
                            for($i=0;$i<$num;$i++){
                                echo '<div class="ball" style="background:-webkit-radial-gradient(center,'.$color.', black)"></div>';
                            }
                        }
                    ?>
                </div>
